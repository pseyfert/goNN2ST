...
def main():
    ...
    gdata = GaussianData(mean_b, cov_b, mean_t, cov_t, n_points=200000)
    goTS = npct.load_library("foo", ".")
    array_1d_double = npct.ndpointer(dtype=np.double, ndim=1, flags='CONTIGUOUS')
    goTS.TS_for_numpy.restype = None
    goTS.TS_for_numpy.argtypes = [array_1d_double, ctypes.c_int, ctypes.c_int, array_1d_double, ctypes.c_int, ctypes.c_int]


    for i in range(1):
        gdata.generate_data(i)
        gdata.scatter_plot()
        ...
        HT.TestStatistic(X_benchmark, X_trial)
        goTS.TS_for_numpy(np.reshape(gdata.x_benchmark,2*200000,'F'),2,200000,np.reshape(gdata.x_trial,2*200000,'F'),2,200000)
